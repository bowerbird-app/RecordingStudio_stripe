...
`/billing` shows **Manage billing on Stripe** above the plan cards when the workspace has a Customer and the actor can `:admin`. The button uses the gem's `:stripe` Flatpack style. The paint is Stripe blurple. Hosts on Flatpack 0.1.189 or newer get `register_style`. The billing view loads `recording_studio_stripe/button` through `content_for :head`. If the host layout does not yield `:head`, link that stylesheet after Flatpack. Each live plan group gets its own card. Standing caps sit with that group's meters, not on the plan card. Cap cards show used of included, including when over. Usage cards show percent used this period. Meter bars stay quiet when nothing is included yet. Extra packs stay off this page. Hosts that still sell them can render `AllowanceCardComponent` and POST `allowances`. `/billing?checkout=ok` explains the wait when Stripe has not written the subscription yet. That POST creates a Stripe Billing Portal session and redirects there. The return URL is the billing page (`success_path`). `:view` can read `/billing` and cannot open the portal. `:edit` cannot pay. Hosts turn the portal on in the Stripe Dashboard. Do not link to dashboard.stripe.com. Do not copy invoices or cards into local tables.
