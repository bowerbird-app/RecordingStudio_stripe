...
class CursorSkillFetchTest < ActiveSupport::TestCase
  test "fetch-skills extras come from the plugin catalog without hardcoded extra URLs" do
    root = RecordingStudioStripe::Engine.root
  ...
  test "gem version is 0.3.1 and gemspec still excludes .cursor" do
    assert_equal "0.3.1", RecordingStudioStripe::VERSION

    spec = Gem::Specification.load(RecordingStudioStripe::Engine.root.join("recording_studio_stripe.gemspec").to_s)
    cursor_files = spec.files.select { |path| path == ".cursor" || path.split("/").include?(".cursor") }

    assert_empty cursor_files

    tracked = `git -C #{RecordingStudioStripe::Engine.root} ls-files -- .cursor/rules`
