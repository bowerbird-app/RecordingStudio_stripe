...
module RecordingStudioStripe
  module Generators
    class InstallGenerator < Rails::Generators::Base
      ...
      desc "Installs RecordingStudioStripe engine into your application"

      class_option(
        :mount_path,
        type: :string,
        default: "/billing",
        desc: "Route prefix used when mounting customer billing"
      )

      def mount_engine
        route %(draw_recording_studio_stripe at: "#{options[:mount_path]}")
      end

      def copy_initializer
        template "recording_studio_stripe_initializer.rb", "config/initializers/recording_studio_stripe.rb"
      end

      def add_yaml_config
        prompt = "Add `config/recording_studio_stripe.yml` for environment-specific settings? [y/N]"
        return unless yes?(prompt)

        template "recording_studio_stripe.yml", "config/recording_studio_stripe.yml"
      end
      ...
      def add_tailwind_source
        ...
        if missing_lines.empty?
          say "Tailwind already configured to include RecordingStudioStripe and FlatPack sources.", :green
          return
        end
      ...
      def inject_tailwind_sources(tailwind_css_path, missing_lines)
        ...
        say "Added RecordingStudioStripe and FlatPack sources to Tailwind CSS configuration.", :green
        say "Run 'bin/rails tailwindcss:build' to rebuild your CSS.", :green
      end

      def formatted_tailwind_source_block(missing_lines)
        engine_lines = missing_lines.grep(/recording_studio_stripe/)
